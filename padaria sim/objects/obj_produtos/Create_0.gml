...
f_array = function()
{
	for (var i = 0; i < array_length(obj.comidas[0]); i++;)
	{
		if (point_in_rectangle(device_mouse_x_to_gui(0), device_mouse_y_to_gui(0), room_width/2-250+(i-floor(i/5)*5)*100-36, 100-36+100*floor(i/5), room_width/2-250+(i-floor(i/5)*5)*100+36, 100+36+100*floor(i/5)))
		{
			...
			{
				for (var j = i; j < array_length(obj.comidas[0]); j++)
				{
					if (j == i)
					{
						global.p_quantidade[obj.comidas[2, j]] += obj.comidas[3, j];
					}
					...
					{
						array_resize(obj.comidas[0], j);
						array_resize(obj.comidas[1], j);
						array_resize(obj.comidas[2], j);
						array_resize(obj.comidas[3], j);
					}
				}
				break;
			}
		}
		draw_sprite_ext(spr_comidas, obj.comidas[2, i], room_width/2-250+(i-floor(i/5)*5)*100, 100+100*floor(i/5), 4, 4, 0, c_white, 1);
		...
		draw_text(room_width/2-250+(i-floor(i/5)*5)*100, 135+100*floor(i/5), "$" + string(obj.comidas[1, i]))
		draw_set_color(c_black);
		draw_text(room_width/2-250+(i-floor(i/5)*5)*100, 100+100*floor(i/5), string(obj.comidas[3, i]));
		draw_set_color(c_white);
		draw_set_halign(-1);
	}
}
