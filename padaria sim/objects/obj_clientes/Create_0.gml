...
f_clientes = function()
{
	for (var i = 7*pagina; i < clamp(array_length(nomes), pagina*7, (pagina+1)*7); i++;)
	{
		...
		draw_text(room_width/2-200, 60+75*(i-floor(i/7)*7), "nome: " + nomes[i]);
		draw_text(room_width/2-200, 75+75*(i-floor(i/7)*7), "idade: " + idades[i]);
		draw_text(room_width/2-200, 90+75*(i-floor(i/7)*7), "xp: " + string(global.c_xp[i]));
		draw_text(room_width/2-200, 105+75*(i-floor(i/7)*7), "dinheiro gasto: " + string(global.c_dinheiro[i]));
	}
}